...
static void allocator_block_pool_free(struct lwis_allocator_block_mgr *block_mgr,
				      struct lwis_allocator_block_pool *block_pool)
{
	...
	if (block_pool->in_use_count != 0 || block_pool->in_use != NULL) {
		pr_err("block_pool %s still has %d block(s) in use\n", block_pool->name,
...
allocator_free_block_get(struct lwis_allocator_block_pool *block_pool)
{
...
static void allocator_free_block_put(struct lwis_allocator_block_pool *block_pool,
				     struct lwis_allocator_block *block)
{
	if (block_pool == NULL) {
		pr_err("block_pool is NULL\n");
...
int lwis_allocator_init(struct lwis_device *lwis_dev)
{
	...
	if (block_mgr == NULL) {
		pr_err("Allocate block_mgr failed\n");
		return -ENOMEM;
	}
	lwis_dev->block_mgr = block_mgr;

	/* Initialize mutex */
	mutex_init(&block_mgr->lock);
...
void lwis_allocator_release(struct lwis_device *lwis_dev)
{
	...
	if (block_mgr == NULL) {
		pr_err("block_mgr is NULL\n");
		return;
	}

	mutex_lock(&block_mgr->lock);
	allocator_block_pool_free(block_mgr, &block_mgr->pool_8k);
	allocator_block_pool_free(block_mgr, &block_mgr->pool_16k);
	allocator_block_pool_free(block_mgr, &block_mgr->pool_32k);
	allocator_block_pool_free(block_mgr, &block_mgr->pool_64k);
	allocator_block_pool_free(block_mgr, &block_mgr->pool_128k);
	allocator_block_pool_free(block_mgr, &block_mgr->pool_256k);
	allocator_block_pool_free(block_mgr, &block_mgr->pool_512k);
	mutex_unlock(&block_mgr->lock);
...
void *lwis_allocator_allocate(struct lwis_device *lwis_dev, size_t size)
{
	...
	*/
	...
	mutex_lock(&block_mgr->lock);
	block = allocator_free_block_get(block_pool);
	mutex_unlock(&block_mgr->lock);
	if (block != NULL) {
		return block->ptr;
	}
...
void lwis_allocator_free(struct lwis_device *lwis_dev, void *ptr)
{
	...
	allocator_free_block_put(block_pool, block);
	if (!in_irq) {
		mutex_unlock(&block_mgr->lock);
	}
